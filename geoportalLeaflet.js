...
L.TileLayer.WMTS = L.TileLayer.extend({
        ...
        getTileUrl: function (tilePoint, zoom) { // (Point, Number) -> String
                ...
                new_url = url + L.Util.getParamString(this.wmtsParams, url) + "&tilematrix=" + ident + "&tilerow=" + tilerow +"&tilecol=" + tilecol ;
                var adjusted_params = {"FORMAT": this.wmtsParams['format'], "LAYER":this.wmtsParams['layer'],"REQUEST":this.wmtsParams['request'],
                "SERVICE":this.wmtsParams['service'],"TILEMATRIXSET":this.wmtsParams['tilematrixSet'],"TIME":this.wmtsParams['time'],
                "VERSION":this.wmtsParams['version']};
                adjusted_url = url +
                    "TIME="+this.wmtsParams['time'] +
                    "&SERVICE="+this.wmtsParams['service'] +
                    "&REQUEST="+this.wmtsParams['request'] +
                    "&VERSION="+this.wmtsParams['version'] +
                    "&LAYER="+this.wmtsParams['layer'] +
                    "&STYLE=&TILEMATRIXSET="+this.wmtsParams['tilematrixSet'] +
                    "&TILEMATRIX="+ident +
                    "&TILEROW="+tilerow +
                    "&TILECOL="+tilecol +
                    "&FORMAT=image%2Fjpeg";
